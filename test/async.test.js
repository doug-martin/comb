...
it.describe("comb.async", function (it) {
    ...
    it.describe(".every", function (it) {
        ...
        it.should("wait for inner promises to complete and return true if all items pass", function () {
            return array(arr).every(function (item, index) {
                assert.equal(item, arr[index]);
                return Promise.resolve(comb.isNumber(item));
            }).then(function (results) {
                assert.isTrue(results);
            });
        });
        ...
        it.should("wait for inner promises to complete and return false if all items do not pass", function () {
            return array(arr).every(function (item, index) {
                assert.equal(item, arr[index]);
                return Promise.resolve(comb.isString(item));
            }).then(function (results) {
                assert.isFalse(results);
            });
        });
    ...
    it.describe(".some", function (it) {
        ...
        it.should("wait for inner promises to complete and return false if no items pass", function () {
            return array(arr).some(function (item, index) {
                assert.equal(item, arr[index]);
                return Promise.resolve(item > 16);
            }).then(function (results) {
                assert.isFalse(results);
            });
        });
    ...
    it.describe(".avg", function (it) {
        ...
        it.should("throw errors for items that are not numbers", function () {
            return array(["A", "B", "C"]).avg().then(assert.fail, function (err) {
    ...
    it.describe("pluck", function (it) {
        it.should("pluck properties", function (it) {
            var arr = [
                ...
                {name: {first: Promise.resolve("Bob"), last: "Yukon"}, age: Promise.resolve(40), roles: Promise.resolve(["b", "c"])},
                {name: {first: "Alice", last: "Palace"}, age: 35, roles: ["c"]},
                {name: {first: Promise.resolve("Johnny"), last: "P."}, age: 56, roles: Promise.resolve([])}
            ];
            return Promise.all([
    ...
    it.describe("#invoke", function (it) {
        it.should("support invoke", function () {
            ...
            var arr = [Promise.resolve(person("Bob", 40)), Promise.resolve(person("Alice", 35)), Promise.resolve(person("Fred", 50)), Promise.resolve(person("Johnny", 56))];
            return Promise.all([
                asyncDeepEqual(comb.async.invoke(arr, "getName"), ["Bob", "Alice", "Fred", "Johnny"]),
                asyncDeepEqual(array(arr).invoke("getOlder").invoke("getAge"), [41, 36, 51, 57])
            ]);
