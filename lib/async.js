    ...
    if (!p || !p.__isArrayAsync__) {
        ...
        forEach(methods, function (m) {
            ...
            ret[m] = function () {
                ...
                return asyncArray(new Promise(function (resolve, reject) {
                    return func.apply(null, [ret].concat(args)).then(resolve, reject);
                }));
            };
        });
...
function _loopResults(cb, scope, results, index, offset, limit) {
    return function () {
        return whenArr(results.slice(offset, limit + offset).map(function (r, i) {
            var ret = new Promise(function (resolve, reject) {
                try {
                    whenArr(cb.apply(scope || results, [r, i + offset, results])).then(resolve, reject);
                } catch (e) {
                    reject(e);
                }
            });
...
function asyncLoop(promise, cb, scope, limit) {
    ...
    return whenArr(promise).then(function (results) {
...
function asyncForEach(promise, iterator, scope, limit) {
    return asyncArray(asyncLoop(promise, iterator, scope, limit).then(function (results) {
        return results.arr;
    }));
}
 ...
 */
function asyncZip() {
    return asyncArray(whenArr.apply(null, argsToArray(arguments)).then(function (result) {
...
function asyncAvg(avgPromise) {
    return whenArr(avgPromise).then(function (result) {
...
function asyncCompact(arr) {
    return asyncArray(whenArr(arr).then(function (result) {
...
function asyncMax() {
    ...
    return whenArr.apply(null, args).then(function (result) {
...
function asyncSort() {
    ...
    return asyncArray(whenArr.apply(null, args).then(function (result) {
...
function asyncSum(arr) {
    return whenArr(arr).then(function (result) {
...
function asyncValuesAt(arrPromise) {
    ...
    return asyncArray(whenArr(arrPromise).then(function (result) {
        return whenArr(valuesAt.apply(array, [normalizeResult(result)].concat(args)));
    }));
}
...
function asyncInvoke(arrPromise) {
    var args = argsToArray(arguments, 1);
    return asyncArray(whenArr(arrPromise).then(function (result) {
        return whenArr(invoke.apply(array, [normalizeResult(result)].concat(args)));
    }));
}